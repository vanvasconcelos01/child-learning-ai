...
DEFAULTS = {
    "nome":"","apelido":"","idade":"","serie":"","escola":"","turno":"","interesses":"","responsavel":"",
    "diagnosticos":[],"outro_diagnostico":"","outras_caracteristicas":"","usa_fontes":False,
    "selected_materials":["Vídeo","Áudio (responsável)","Slides"],"cobranca_escola":[],"cobranca_extra":""
}
for k,v in DEFAULTS.items():
    st.session_state.setdefault(k, v)

def formatar_data_br(d): return d.strftime("%d/%m/%Y")
def formatar_data_extenso(d): return f"{d.day} de {PT_MONTHS[d.month]} de {d.year}"
...
st.title("🧠 EduAI Studio - v5.8")
st.caption("Visual mais leve, menos cara de formulário, com perfil salvo na página e Studio organizado.")

tabs = st.tabs(["👦 Perfil", "🗓️ Cronograma", "⚙️ Configuração", "🎬 Studio", "📦 Aula Completa"])
...
with tabs[0]:
    ...
    st.session_state["outras_caracteristicas"] = st.text_area("Outras características (editável)", value=st.session_state["outras_caracteristicas"], height=120)
...
with tabs[2]:
    ...
    st.session_state["cobranca_escola"] = st.multiselect("Selecione os formatos mais comuns", ESCOLA_COBRANCA_OPTIONS, default=st.session_state["cobranca_escola"])
...
with tabs[3]:
    ...
    with c1:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("Vídeo")
        st.text_area("Prompt de vídeo", value=prompt_video(perfil,materia,conteudo,estilo,situacao,prioridade,days,usa), height=190)
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("Slides")
        st.text_area("Prompt de slides", value=prompt_slides(perfil,materia,conteudo,estilo,situacao,prioridade,days,usa), height=190)
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("Flashcards")
        st.text_area("Prompt de flashcards", value=prompt_flash(perfil,materia,conteudo,estilo,situacao,prioridade,days,usa), height=190)
        st.markdown('</div>', unsafe_allow_html=True)

    with c2:
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("Áudio do responsável")
        st.text_area("Prompt de áudio", value=prompt_audio(perfil,materia,conteudo,estilo,situacao,prioridade,days,usa), height=190)
        st.markdown('</div>', unsafe_allow_html=True)

        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.subheader("Teste")
        st.text_area("Prompt de teste", value=prompt_teste(perfil,materia,conteudo,estilo,situacao,prioridade,days,usa), height=190)
        st.markdown('</div>', unsafe_allow_html=True)
